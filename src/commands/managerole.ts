...
const _slashCmdRun: SlashCommandFunction = async (interaction) => {
	...
	const roleName = _role?.type === ApplicationCommandOptionType.String ? (_role.value as string) : undefined;
	const action = _action?.type === ApplicationCommandOptionType.String ? (_action.value as 'set' | 'getall') : undefined;
	const key = _key?.type === ApplicationCommandOptionType.String ? (_key.value as string) : undefined;
	const value = _value?.type === ApplicationCommandOptionType.String ? (_value.value as string) : undefined;

	const role = await prisma.teamRole.findFirst({ where: { name: roleName, team: { guild: { guildId: interaction.guildId! } } } });

	if (!roleName || !action) {
		await interaction.reply({ content: 'Missing required arguments', ephemeral: true });
		return;
	}
	if (!role) {
		await interaction.reply({ content: `Role ${roleName} does not exist in this guild`, ephemeral: true });
		return;
	}
		...
		await interaction.reply({
			embeds: [
				new EmbedBuilder()
					...
					.setDescription(rules.map(([key, value]) => `**${key}:** ${value ? 'yes' : 'no'}`).join('\n')),
			],
			ephemeral: true,
		});
		return;
	...
	if (action === 'set') {
		...
		await interaction.reply({
			embeds: [
				new EmbedBuilder().setTitle(`${newRole.name}'s settings`).setDescription(
