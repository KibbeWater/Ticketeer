...
const _slashCmdRun: SlashCommandFunction = async (interaction) => {
  ...
  const team =
    _team?.type === ApplicationCommandOptionType.String
      ? (_team.value as string)
      : undefined;
  const name =
  ...
  const teams = await prisma.team.findMany({
    where: { name: team, guild: { guildId: interaction.guildId! } },
  });
...
const _textRun: TextCommandFunction = async (msg, args) => {
  ...
  const teams = await prisma.team.findMany({
    where: { name: teamName, guild: { guildId: msg.guildId! } },
  });
